...
const hero = {
  ...
  image: '/images/banners/personalized-training.jpg', // <- replace with your actual asset
  // schedule: 'Flexible',
  // mode: '1:1 or Small Group',
}

// If a react-icon is missing, fall back to a neutral box
...
// Use Devicon (multi-color brand) for these keys
const USE_DEVICON = new Set(['aws', 'java', 'nodejs'])

// Devicon class map (multi-color)
const DEVICON_CLASS = {
...
// Brand colors for mono simple-icons
const BRAND = {
...
// Courses list (popular items rise to top)
const COURSES = [
  // Popular
  { key: 'python', name: 'Python', href: '/training/python', Icon: SiPython, popular: true },
  { key: 'java', name: 'Java', href: '/training/java', Icon: null, popular: true },        // devicon
  { key: 'nodejs', name: 'Node.js', href: '/training/nodejs', Icon: null, popular: true }, // devicon
  { key: 'react', name: 'React', href: '/training/react', Icon: SiReact, popular: true },
  { key: 'aws', name: 'AWS', href: '/training/aws', Icon: null, popular: true },           // devicon
  { key: 'power-bi', name: 'Power BI', href: '/training/power-bi', Icon: SiPowerbi, popular: true },
  { key: 'sql-server', name: 'SQL Server', href: '/training/sql-server', Icon: SiMicrosoftsqlserver, popular: true },
  { key: 'mysql', name: 'MySQL', href: '/training/mysql', Icon: SiMysql, popular: true },
  { key: 'docker', name: 'Docker', href: '/training/docker', Icon: SiDocker, popular: true },
  { key: 'kubernetes', name: 'Kubernetes', href: '/training/kubernetes', Icon: SiKubernetes, popular: true },

  // More
  { key: 'c', name: 'C', href: '/training/c', Icon: SiC },
  { key: 'cpp', name: 'C++', href: '/training/cpp', Icon: SiCplusplus },
  { key: 'angular', name: 'Angular', href: '/training/angular', Icon: SiAngular },
  { key: 'azure', name: 'Microsoft Azure', href: '/training/azure', Icon: SiMicrosoftazure },
  { key: 'css', name: 'CSS3', href: '/training/css', Icon: SiCss3 },
  { key: 'django', name: 'Django', href: '/training/django', Icon: SiDjango },
  { key: 'dotnet', name: '.NET', href: '/training/dotnet', Icon: SiDotnet },
...
// Renders either a Devicon (multi-color) or react-icon (mono with brand hex)
function CourseIcon({ keyName, Icon }) {
  if (USE_DEVICON.has(keyName)) {
    return (
      <i
        className={`${DEVICON_CLASS[keyName]} text-[40px] leading-none`}
        aria-hidden
      />
    )
  }
 ...
 <section className="w-full bg-black py-20 px-6">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-10 items-center">
          ...
          <div>
            ...
            {/* <p className="text-sm text-white mb-4">
              <strong>Schedule:</strong> {hero.schedule} &nbsp;•&nbsp; <strong>Mode:</strong> {hero.mode}
            </p> */}
            <Link
              href="/request-callback"
      ...
      <section className="px-6 py-10">
        <div className="mx-auto max-w-7xl">
          <h2 className="text-2xl md:text-4xl text-center  font-bold tracking-tight text-blue-600">Course Catalog</h2>
        
    

          <ul className="mt-6 grid gap-6 grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">
            {SORTED.map(({ key, name, href, Icon }) => (
              <li key={key}>
                <Link
                  href={href}
                  aria-label={name}
                  className="group block rounded-2xl border border-gray-100 bg-white p-6 shadow-sm
                             transition hover:-translate-y-0.5 hover:shadow-md focus:outline-none
                             focus-visible:ring-2 focus-visible:ring-blue-500/40"
                >
                  <div className="grid place-items-center">
                    <div className="grid h-20 w-20 place-items-center rounded-2xl bg-gray-50 ring-1 ring-gray-200">
                      <CourseIcon keyName={key} Icon={Icon} />
                    </div>
      ...
      <section className="px-6 py-12 bg-gray-50">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {FAQ.map(({ q, a }) => (
              <div key={q} className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                <div className="text-sm font-semibold text-gray-900">{q}</div>
                <p className="mt-1 text-sm text-gray-600">{a}</p>
              </div>
            ))}
